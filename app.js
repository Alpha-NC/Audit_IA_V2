...
function transformAnalysisROI(htmlContent) {
  ...
  const timeWastedWeekly = state.answers.time_wasted_weekly || 0;
  ...
  if (estimatedSavingsDaily >= 1) {
    ...
    gainText = `~${minutes} min / jour`;
  }
  
  // Remplacer les patterns ROI % dans le HTML
  // Pattern 1: "ROI Estimé : XX%" ou "ROI estimé : XX%"
  let transformed = htmlContent.replace(
    /ROI\s+[Ee]stim[ée]\s*:\s*\d+%/gi,
    `Gain estimé : ${gainText}`
  );
  ...
  // Pattern 3: éléments avec classe contenant "roi" et valeur en %
  // On remplace aussi les labels "ROI" par "Gain temps"
  transformed = transformed.replace(
    />ROI</gi,
    '>Gain temps<'
  );
